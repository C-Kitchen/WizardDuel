...
class Ball {
   ...
   */
  public Ball() {
    this (new PVector (random(300), random(300)), new PVector (0, 0), random(20)+5, -1, 1);
  }
  ...
  void update() {
    ...
    if (friction) {
      ...
      velocity.mult (velocity.magSq() < .05 ? 0 : f);
    }
...
class Orb extends Ball {
  ...
  public Orb(float xi, float yi, float xSpeedi, float ySpeedi) {
    super(xi, yi, xSpeedi, ySpeedi, 5, (int) Math.pow(2, (int)random(3)) + ((xSpeedi == 0 && ySpeedi == 0)?7:11));
  }
  ...
  void reset() {
     ...
     */
    boolean good = true;
    //this loop makes sure it's not too close to the players
    do {
      good = true;
    ...
    kind = (int)Math.pow(2, (int)random(3)) + 7;
  }
...
class Hat extends Ball {
  ...
  public Hat(Wizard myWizardi) {
    ...
    super (PVector.add(myWizardi.getPosition(), new PVector (-4*myWizardi.getSize(), -54*myWizardi.getSize())), PVector.mult(myWizardi.getVelocity(), 1), 11, 10);
    myWizard = myWizardi;
    angle = -.2;
...
class Wizard extends Ball {
  ...
  /*
    control: 0 = CPU, 1 = mouse, 2 = keyboard
  ...
  boolean collides (int otherKind) {
    if (kind == 16) {
      ...
      //add an orb if you touch one that you don't have
      kind += (otherKind % 4) + 1;
...
void splitL (float x, float y, float w, float h, float stoneX, float stoneY) {
  ...
  if (horizontal) {
    int newStoneX = (int) (w - abs(stoneX) < minStone * 2 ? (int) w - abs(stoneX) : (int) random(minStone, w - abs(stoneX) - minStone + 1));
    ...
    int newStoneY = (int) (h - abs(stoneY) < minStone * 2 ? h - abs(stoneY) : (int) random(minStone, h - abs(stoneY) - minStone + 1));
...
void startGame() {
  ...
  balls[i*2+1] = new Hat((Wizard) balls[i*2]);
  }
...
void setup() {
  ...
  players = 2;
  //christmas mode automatically comes on if month is december
  christmas = ((Calendar.getInstance()).get(Calendar.MONTH) == 11);
  ...
  titleHat = new Hat (width/2-128, height/2-17, .8, .7, true);
  strokeWeight(1);
